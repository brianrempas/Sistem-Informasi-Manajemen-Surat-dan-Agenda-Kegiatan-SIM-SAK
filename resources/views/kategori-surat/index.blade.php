<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        {{-- Left: Search --}}
        <form method="GET" action="{{ route('kategori-surat.index') }}"
            class="flex flex-col sm:flex-row gap-3 flex-wrap w-full md:w-auto">

            <input name="search" placeholder="Cari nama kategori..." value="{{ request('search') }}"
    ...
    <div class="bg-white shadow rounded-lg overflow-x-auto">
        <table class="w-full min-w-[600px] table-auto">
            <thead class="bg-gray-100 text-left text-sm text-gray-700">
                <tr>
                    <x-table-sortable-header label="Nama Kategori" sortField="nama_kategori" />
            ...
            <tbody>
                @forelse ($items as $kategori)
                    <tr class="border-t">
                        <td class="p-3 break-words">{{ $kategori->nama_kategori }}</td>
                        <td class="p-3 break-words text-gray-700">{{ $kategori->deskripsi ?? '-' }}</td>
                        <td class="p-3 flex gap-2 justify-center">
                            <a href="{{ route('kategori-surat.edit', $kategori) }}" class="text-yellow-600">Edit</a>
                            <form action="{{ route('kategori-surat.destroy', $kategori) }}" method="POST">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="3" class="text-center p-3 text-gray-400">Belum ada data kategori.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
